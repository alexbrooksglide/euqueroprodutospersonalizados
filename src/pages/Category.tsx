...
const Category = () => {
  ...
  const list = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => sort === "menor" ? a.price - b.price : sort === "maior" ? b.price - a.price : sort === "recentes" ? b.createdAt - a.createdAt : 0);
  ...
  return (
    <main className="animate-fade-in px-5 pb-10 pt-[calc(env(safe-area-inset-top)+1.25rem)]">
      ...
      {filtersOpen && <div className="mt-3 rounded-2xl border border-border bg-muted p-4"><p className="mb-3 text-sm font-bold">Filtrar produtos</p><div className="grid grid-cols-2 gap-2">{[["preço", "Faixa de preço"], ["categoria", "Categoria"], ["disponibilidade", "Disponíveis"], ["novidades", "Novidades"], ["vendidos", "Mais vendidos"], ["promoções", "Promoções"]].map(([value, label]) => <button key={value} onClick={() => toggle(value)} className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-left text-xs ${selected.includes(value) ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}>{selected.includes(value) && <Check className="h-3 w-3" />}{label}</button>)}</div></div>}
